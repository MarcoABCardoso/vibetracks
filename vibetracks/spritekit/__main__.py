"""VibeTracks spritekit CLI: validate sprite specs and composite them to PNG.
...
and the ``assets/`` LPC art it composites. Mirrors the audio CLI:
    ...
    python -m vibetracks.spritekit validate                    # check every cast
    python -m vibetracks.spritekit render knight-guild/knight  # composite one -> out/
    python -m vibetracks.spritekit render-all                  # every character
    python -m vibetracks.spritekit new <char> --cast <c>       # scaffold a character
    python -m vibetracks.spritekit new-cast <name>             # scaffold a whole cast
...
def cmd_new(args) -> int:
    ...
    print(f"scaffolded {path} — edit it, then: "
          f"python -m vibetracks.spritekit render {c.name}/{args.name}")
    return 0
...
def cmd_new_cast(args) -> int:
    ...
    print(f"  render:     python -m vibetracks.spritekit render-all --cast {args.name}")
    return 0


def main(argv=None) -> int:
    p = argparse.ArgumentParser(prog="vibetracks.spritekit", description=__doc__,
